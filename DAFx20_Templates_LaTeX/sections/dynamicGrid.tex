...
Though the paper shows changes in the wavespeed $c$ rather than the length $L$, the effect of a change in either of these parameters has an identical effect on the system \SWcomment[as long as the geometry is unchanged for the grid points.] This also means that grid spacing $h$ not change during the simulation, but rather the spatial domain of the system.
