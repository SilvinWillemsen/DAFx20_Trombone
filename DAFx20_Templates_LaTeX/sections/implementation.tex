...
As the tube acts mainly as an amplifier for specific resonant frequencies it is important to match the frequency of the lip reed to a resonating mode of the tube (which depends on $L^n$). This is done according to
\begin{equation}
...
To reduce audible artefacts and instability issues form adding and removing points \SWcomment[also to stay in the sub-audio rate regime], a limit can be placed on \eqref{eq:lDiff} as
...
\SWcomment[check centered and centred (both papers)]
...
\begin{algorithm}[ht]
    ...
    \fbox{\parbox{0.88\linewidth}
    {
        \While{application is running}
        {
            ...
            Calc. $U_\text{B}$ and $U_\text{r}$ (Eqs. \eqref{eq:bernoulli} and \eqref{eq:Ur})\\
